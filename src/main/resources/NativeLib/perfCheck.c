...
static perf_event_attr_t *perf_event_attrs; //initialized in perfEventInit
...
//inits the perf event attr array. an array of "attrs" which is a structure that holds data about a performance event
JNIEXPORT void JNICALL Java_PerfCheckUtils_perfInit(JNIEnv * env, jclass jcls, jint numEvents, jint isSet) {
	setGroup(isSet);
	num_events = numEvents;
	int i;
	int ret = fm_initialize();
	if (ret != PFM_SUCCESS) {
